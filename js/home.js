...
function showForgotPasswordForm() {
  ...
  document.getElementById("regiseter-form-area").style.display = "none";
...
function submitForgotPassword() {
  ...
  if (email) {
    ...
    .then(data => {
      if (data.success) {
        alert("Email lấy lại mật khẩu đã được gửi đến: "+ email);
      } else {
...
window.onload = function () {
  ...
  // Nếu có token, hiển thị form đặt lại mật khẩu và ẩn các form khác
  if (token) {
    document.getElementById("login-form-area").style.display = "none";
    document.getElementById("regiseter-form-area").style.display = "none";
    document.getElementById("forgot-password-area").style.display = "none";
...
// Hàm gửi mật khẩu mới lên server khi người dùng nhấn "Cập Nhật Mật Khẩu"
...
function renderProfileDetail(account) {
  ...
  function updateUserProfile() {
    account.fullname = p_name.value;
    account.username = p_username.value;
...
let filter_letter = document.getElementById("filter-letter");  
let allF_product = document.getElementById("all-product"); // Giả sử bạn đã định nghĩa biến này để chứa danh sách sản phẩm  

filter_option.addEventListener("change", filterProduct);  
filter_letter.addEventListener("change", filterProduct);  

function filterProduct() {  
  // Sao chép danh sách sản phẩm gốc để không làm thay đổi dữ liệu gốc  
  const filteredProducts = PRODUCTS.slice();  

  // Lọc theo chữ cái đầu  
  const letterOption = filter_letter.value;  
  if (letterOption) {  
    filteredProducts = filteredProducts.filter(product =>   
      product.name.startsWith(letterOption)  
    );  
  }  

  // Lọc theo giá  
  const priceOption = filter_option.value;  
  if (priceOption === "priceUp") {  
    filteredProducts.sort((p1, p2) => p1.price - p2.price);  
  }  
  if (priceOption === "priceDown") {  
    filteredProducts.sort((p1, p2) => p2.price - p1.price);  
  }  

  // Cập nhật danh sách sản phẩm hiển thị  
  allF_product.innerHTML = ""; // Xóa nội dung cũ  
  filteredProducts.forEach(product => renderAllProduct(product)); // Hiển thị sản phẩm đã lọc  
}
...
let customer_name = document.getElementById("customer-fullname");
let customer_username = document.getElementById("customer-username");
...
function renderUserInfo(account) {
  customer_name.value = account.fullname;
  customer_username.value = account.username;
